      ...
      <header className="sticky top-0 z-40 backdrop-blur-xl border-b" style={{ background: ct.headerBg, borderColor: ct.border }}>
        <div className="container mx-auto px-4 sm:px-6 py-3 sm:py-4">
          <div className="flex flex-col sm:flex-row sm:justify-between sm:items-center gap-2 sm:gap-0">
            {/* Row 1: actions + progress */}
            <div className="flex items-center justify-between sm:justify-start sm:gap-5">
              <Link to="/escape-game" className="hover:opacity-80 transition flex items-center gap-1.5 text-xs sm:text-sm" style={{ color: ct.accent, fontFamily: ct.font }}>
              ...
              >
                ...
                {isTainan || isHualien ? '開場故事' : 'PROLOGUE'}
              </button>
              <div className="sm:hidden text-[11px]" style={{ color: ct.accent, fontFamily: ct.font }}>
                <span style={{ opacity: 0.7 }}>{ct.label.progress}</span> {completedMissions.length}/{game.missions.length}
              </div>
            </div>
            {/* Row 2 on desktop: agent + progress */}
